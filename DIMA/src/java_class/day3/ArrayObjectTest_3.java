...
public class ArrayObjectTest_3 {
	...
	public static void main(String[] args) {
		...
		// for(Human h: hAry2) System.out.println(h); // h는 주소를 가진다. -> h.name h를 따라가서
		// 값을 읽어온다. (직접 접근 - 원래 이렇게 하면 X)
		for (Human h : hAry2)
			System.out.println(h.name + " / " + h.age); // 객체 지향언어에서 직접 접근은 비권장 (하지만 가능은 하다)
